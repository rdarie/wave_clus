...
% SPC PARAMETERS 
par.mintemp = 0.05;                  % minimum temperature for SPC 
par.maxtemp = 0.25;                 % maximum temperature for SPC 
par.tempstep = 0.01;                 % temperature steps 
par.SWCycles = 200;                  % SPC iterations for each temperature (default 100) 
...
par.notch_wid = 5;                 % half width of band stop filter for detection (default 1000) 
par.detect_order = 5;                % filter order for detection 
par.notch_order = 3;                % filter order for detection 
...
par.template_type = 'center';       % nn, center, ml, mahal, carlos 
...
par.max_spk = 200000;               % max. # of spikes before starting templ. match. 
par.permut = 'n';                   % for selection of random 'par.max_spk' spikes before starting templ. match. 
 
% par.permut = 'y';                 % for selection of the first 'par.max_spk' spikes before starting templ. match. 
 
% HISTOGRAM PARAMETERS 
 
par.nbins = 150;                    % # of bins for the ISI histograms 
 
par.bin_step = 2;                   % percentage number of bins to plot 
